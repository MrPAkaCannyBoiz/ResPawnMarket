...
public class GetCustomerGrpcService : IGetCustomerService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new KeyNotFoundException($"Customer with ID {request.CustomerId} not found.", ex);
        }
    ...
    public async Task<GetAllCustomersResponse> GetAllCustomerAsync(GetAllCustomersRequest request,
        CancellationToken cancellationToken = default)
    {
        try
        {
            var response = await _grpcClient.GetAllCustomersAsync(request,
                cancellationToken: cancellationToken);
            return response;
        }
        catch (RpcException ex) when (ex.StatusCode == StatusCode.NotFound)
        {
            throw new KeyNotFoundException($"All customers not found.", ex);
        }
        catch (RpcException ex)
        {
            throw new ApplicationException($"gRPC {ex.StatusCode}: {ex.Status.Detail}", ex);
